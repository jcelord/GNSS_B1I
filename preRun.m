...
% The acquiredsignals are sorted according to the signal strength.
% This function can be modified to use other satellite selection algorithms
% or to introduceacquired signal properties offsets for testing purposes.
%
...
%
%   Outputs:
%       channel     - structure contains information for each channel
%                          (like properties of the tracked signal, channel status etc.). 
...
%CVS record: preRun.m,v 1.8.2.20 2006/08/14 11:38:22 dpl Exp $
...
channel.status = '-';                                             % Mode/status of the tracking channel£¬"-":no signal to track,"T":Tracking state
...
% but not more as the number of channels specified in the settings.
