...
B1ICodesTable = makeB1Table(settings);
...
%% =================== Correlate signals ==============================
    ...
    caCodeFreqDom = conj(fft(B1ICodesTable(PRN,:)));

    % Make the correlation for whole frequency band (for all freq. bins)
    for frqBinIndex = 1:numberOfFrqBins

        % Generate carrier wave frequency grid (0.5kHz step)
        frqBins(frqBinIndex) = settings.IF -  (settings.acqSearchBand/2) * 1000 +  500 * (frqBinIndex - 1);
        ...
        % Perform inverse DFT and store correlation result
        acqRes1 = abs(ifft(convCodeIQ1)) .^ 2;
        acqRes2 = abs(ifft(convCodeIQ2)) .^ 2;
        
        % Check which msec had the greater power and save that,
        % will "blend" 1st and 2nd msec but will correct data bit issues
...
%% ============= Fine resolution frequency search ==========================
        ...
        b1iCode = codegen_B1I(PRN);
        codeValueIndex = floor((ts * (1:10*samplesPerCode)) / (1/settings.codeFreqBasis));        
        longCaCode = b1iCode((rem(codeValueIndex, 1023) + 1));
    
        % Remove C/A code modulation from the original signal (Using detected C/A code phase)
        ...
        %acqResults.carrFreq(PRN)  = fftFreqBins(fftMaxIndex) - settings.IF;
